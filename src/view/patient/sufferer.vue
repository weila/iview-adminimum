<template>
  <div>
    <Form ref="searchForm" class="form-item__inline" :model="searchForm" inline>
      ...
      <FormItem label="患者ID">
        <Input v-model="searchForm.patientId"/>
      </FormItem>
      <FormItem label="机构">
      ...
      <div class="split-pane-page-wrapper">
        <split-pane v-model="splitPaneOffset">
          <div slot="left" class="pane left-pane">
            <Card :bordered="false">
              <p slot="title">已选项</p>
              <Table :columns="selectionColumns" :data="selection">
          ...
          <div slot="right" class="pane right-pane">
            <Card :bordered="false">
              ...
              <Input style="padding-bottom: 4px;" v-model="searchMainIndex.name">
                <span slot="prepend">姓名</span>
              </Input>
              <Input style="padding-bottom: 4px;" v-model="searchMainIndex.deptName">
                <span slot="prepend">科室</span>
              </Input>
              <Input style="padding-bottom: 4px;" v-model="searchMainIndex.healthCareProviderId">
...
import Config from './config-sufferer'
...
export default {
  ...
  methods: {
    // 查询
    query(page = 1) {
      const { globalId, identityNo, patientId, domainId, patientName } = this.searchForm || ''
      ...
      hipMasterPost(data).then(res => {
        ...
        for (let i in this.results) {
          for (let j in this.pageAndSelection[this.current]) {
            if (this.results[i].pk === this.pageAndSelection[this.current][j].pk) {
              this.results[i]._checked = true
            }
          }
        }
      })
    ...
    showCombineAndQueryMainIndex(page = 1) {
      ...
      const data = {
        ...
        'payLoad': {
          'healthCareProviderId': this.searchMainIndex.healthCareProviderId,
          'deptName': this.searchMainIndex.deptName,
          'globalId': this.searchMainIndex.globalId,
          'name': this.searchMainIndex.name,
          'pageNumber': thePage,
          'pageSize': 10
        }
      }
      hipMasterPost(data).then(res => {
        ...
        this.mainIndex = res.data.records
        this.mainIndexTotal = res.data.total
        this.mainIndexCurrent = res.data.current
      })
    },
    // 选择条目
